...
static void batt_task(void *arg) { while(1) { read_battery(); ESP_LOGD(TAG,"%umV=%u%%",s_mv,s_pct); vTaskDelay(pdMS_TO_TICKS(30000)); } }
void rl_battery_init(void) {
    ...
    xTaskCreate(batt_task,"rl_battery",2048,NULL,3,NULL);
}
